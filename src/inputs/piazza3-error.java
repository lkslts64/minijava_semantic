...
class B extends A{
    ...
    public int foo() {int x;
		...
		A a2;
		A a3;
		a2 = (type.dfoo(true, 1, 2, type)).ffoo(type);
		a3 = type.ffoo(type);
        a = type.dfoo(
                type.foo(),
                1,
                type.foo(),
                ((type.ffoo(type)).ffoo(type)).ffoo( (type.dfoo(1, 1, 2, type.ffoo(type))).dfoo(type.bfoo(1,2), type.cfoo(type.afoo(1), 1, 3), 3, type.ffoo(type))) );
    return 0;}
